...
////this api is used to store the code. suppose wwww.idp.sust.com generates a code "123456" for wwww.sp1.sust.com. here forWhichSp is www.sp1.sust.com, whichIdp is www.sust.com and code is 123456
        app.post('/storecode', async(req, res) => {
            try{
               ...
               // console.log(spentityid,idpentityid,idpcode)
                await contract.submitTransaction('NewStoreCode', spentityid, idpentityid, spCode, idpCode, spCheck, idpCheck, author);
                res.send('Success')
            }catch (error) {
        ...
        app.get('/approval', async(req, res) => {
            try{
                ...
                console.log('////////////////////////////////')
                console.log(author)
            let result = await contract.evaluateTransaction('Approval', author);
        ...
        app.get('/newcodefetch', async(req, res) => {
            try{
                // let user = req.body.user
                // let metaData = req.body.metaDa
                let spentityid = req.query.spentityid;
                let idpentityid = req.query.idpentityid
                let author = req.query.author;
                let code = req.query.code
                console.log('////////////////////////////////')
                console.log(spentityid, idpentityid, author, code)
            let result = await contract.evaluateTransaction('NewCode', spentityid, idpentityid, author, code);
           // console.log('GET approval : ', result);   
            res.send(JSON.stringify(result.toString()))
            }catch (error) {
                console.error(`Failed to evaluate transaction last: ${error}`);
        ...
        app.get('/removeapproval',async(req, res) => {
            try{      
                // let user = req.body.user
                // let metaData = req.body.metaData
                let spentityid = req.query.spentityid;
                let idpentityid = req.query.idpentityid;
         await contract.submitTransaction('RemoveApproval', spentityid, idpentityid);
               res.send('Success')
             ///res.send(JSON.stringify(result.toString()))
        ...
        app.get('/codefetch', async(req, res) => {
            try{nl
               ...
               // var r =parseInt((result.toString()))
                console.log(result)
        ...
        app.get('/deletecode', async(req, res) => {
            try{
             ...
             await contract.submitTransaction('DeleteCodeSp', spentityid, idpentityid);
             res.send('Success')
            }catch (error) {
                console.error(`Failed to submit transaction store Code: ${error}`);
                process.exit(1);
...
///this api return the trusted anchor list in array. you have to give the entity id whose trusted list you want to see
        app.get('/tallistfetch', async(req, res) => {
            try{
                ...
                // let metaData = req.body.metaData
               let result=  await contract.evaluateTransaction('TalListReturn', entityId);
                    console.log(result.toString())
        ...
        app.get('/showtallist', async(req, res) => {
            try{
               // let entityId = req.params.entityId
                // let metaData = req.body.metaData
               let result=  await contract.evaluateTransaction('ShowTalList');
                    console.log(result.toString())
                    let resultJson = JSON.parse(result.toString())
                    // let arr = []
                    // for (var i in resultJson)
                ...
                console.error(`Failed to evaluate transaction show tall list: ${error}`);
                process.exit(1);
            }
        
        });

        //// This api is used to delete the entity id from the sp/idp trusted anchor list
        app.get('/deletetal', async(req, res) => {
            try{
                let entityid = req.query.entityid
                let tal = req.query.tal

             await contract.submitTransaction('TalListDelete', entityid, tal);
             res.send('Success')
            }catch (error) {
                console.error(`Failed to submit transaction store Code: ${error}`);
                process.exit(1);
